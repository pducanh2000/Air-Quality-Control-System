...
void setup(){ 
  ...
  dht.begin();
  // every 4 seconds, check WiFi connection and try to connect if not connected 
  timer.every(4000, connectWiFi);
...
void sendToThingspeak(){
  // if NodeMCU is connected to Thingspeak server
  if (client.connect(apiServer, 80)){
    pmsSerial.println("THINGSPEAK");
    String str_tmp = String(temperature, 2);
    String str_hum = String(humidity, 2);
    String str_pm25 = String(PM_2_5);
    String str_pm10 = String(PM_10);
    // form a POST request to Thingspeak server
    String postStr = String("api_key=")+String(apiKeyChannel)+"&field1="+str_tmp+"&field2="+str_hum+"&field3="+str_pm25+"&field4="+str_pm10;
